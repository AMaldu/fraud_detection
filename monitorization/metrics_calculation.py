...
def generate_synthetic_dates(df, start_date, hours_interval=1):
    num_rows = len(df)
    date_range = [start_date + timedelta(hours=i * hours_interval) for i in range(num_rows)]
    df['date'] = date_range
    return df
...
def calculate_metrics_postgresql(curr, step):
    try:
        ...
        start_date = datetime.now().replace(minute=0, second=0, microsecond=0) - timedelta(days=5)
        ...
        for _, row in raw_data.iterrows():
            ...
            ( prediction_drift, num_drifted_columns, share_missing_values, row['date'])
        )
